...
for fixed M, n, and cn, based on pivot-table CSVs of the form:
...
#TO run: python3 analysis_best_omega_by_N_vs_sn.py --base_dir output_data --M 10 --n 5 --cn 0.2
...
def find_N_dirs(base_dir: str, M: int, n: int, cn: float) -> List[Tuple[int, str]]:
    """
...
def build_heatmap(base_dir: str, M: int, n: int, cn: float, csv_name: str, verbose: bool=False):
    """
    Scan base_dir for experiments matching M, n, cn, and compute best omega per (N, sn).
    Returns:
      Ns_sorted, sns_sorted_union, omega_matrix (shape len(Ns) x len(sns))
    """
    N_dirs = find_N_dirs(base_dir, M, n, cn)
    if verbose:
        print(f"Found {len(N_dirs)} N directories for M={M}, n={n}, cn={cn} in '{base_dir}'.")
...
def main():
    ...
    plot_heatmap(df_out, args.out_png,
                 title=f"Best ω by (N, sn) | M={args.M}, n={args.n}, cn={args.cn}")

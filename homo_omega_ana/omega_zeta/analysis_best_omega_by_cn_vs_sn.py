...
"""
Build a heatmap of the best omega (personal_info_weight) across (cn, sn)
for fixed N, M, and n, based on pivot-table CSVs of the form:
...
For each cn, we iterate over every available sn column in its CSV and choose
the row with the highest performance, returning the corresponding omega.
We then place that omega into the heatmap cell at (row=cn, col=sn).

Outputs:
- best_omega_by_cn_vs_sn.csv (rows=cn, columns=sn, values=omega)
- best_omega_by_cn_vs_sn.png (imshow heatmap of omega)
"""
...
# To run: python3 analysis_best_omega_by_cn_vs_sn.py --base_dir output_data --N 100 --M 10 --n 5

def parse_args():
    p = argparse.ArgumentParser(description="Build heatmap of best omega by (cn, sn).")
    ...
    p.add_argument("--out_csv", type=str, default="best_omega_by_cn_vs_sn.csv",
                   help="Output CSV filename for the heatmap values (omega)")
    p.add_argument("--out_png", type=str, default="best_omega_by_cn_vs_sn.png",
...
def find_cn_dirs(base_dir: str, N: int, M: int, n: int) -> List[Tuple[float, str]]:
    """
    Return list of (cn, cn_dir_path) for directories matching:
...
def build_heatmap(base_dir: str, N: int, M: int, n: int, csv_name: str, verbose: bool=False):
    """
    Scan base_dir for experiments matching N, M, n, and compute best omega per (cn, sn).
    Returns:
      cns_sorted, sns_sorted_union, omega_matrix (shape len(cns) x len(sns))
    """
    cn_dirs = find_cn_dirs(base_dir, N, M, n)
    if verbose:
        print(f"Found {len(cn_dirs)} cn directories for N={N}, M={M}, n={n} in '{base_dir}'.")

    # First pass: gather union of all sn columns
    sns_union = set()
    per_cn_data: Dict[float, Tuple[List[float], List[float]]] = {}

    for cn_val, cn_dir in cn_dirs:
        sns, best_omegas = read_best_omegas_by_sn(cn_dir, csv_name)
        per_cn_data[cn_val] = (sns, best_omegas)
    ...
    cns_sorted = sorted(per_cn_data.keys())
    sns_sorted_union = sorted(sns_union)

    # Build matrix, fill with NaN where a given cn lacks that sn
    omega_mat = np.full((len(cns_sorted), len(sns_sorted_union)), np.nan, dtype=float)

    for i, cn_val in enumerate(cns_sorted):
        sns, best_omegas = per_cn_data[cn_val]
    ...
    return cns_sorted, sns_sorted_union, omega_mat


def save_csv(cns: List[float], sns: List[float], omega_mat: np.ndarray, out_csv: str):
    df_out = pd.DataFrame(omega_mat, index=cns, columns=sns)
    df_out.index.name = "cn"
...
def plot_heatmap(df_out: pd.DataFrame, out_png: str, title: str=None):
    ...
    ax.set_ylabel("communication noise (cn / η)")
    ax.set_title(title or "Best ω (personal_info_weight) by (cn, sn)")
...
def main():
    ...
    cns, sns, omega_mat = build_heatmap(
    ...
    df_out = save_csv(cns, sns, omega_mat, args.out_csv)
    plot_heatmap(df_out, args.out_png,
                 title=f"Best ω by (cn, sn) | N={args.N}, M={args.M}, n={args.n}")

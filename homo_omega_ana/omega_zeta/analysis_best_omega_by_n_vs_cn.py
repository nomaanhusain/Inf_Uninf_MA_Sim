...
# To run: python3 analysis_best_omega_by_n_vs_cn.py --base_dir output_data --N 100 --M 10 --sn 0.2
...
def parse_args():
    ...
    p.add_argument("--out_csv", type=str, default="best_omega_by_n_vs_cn.csv")
    p.add_argument("--out_png", type=str, default="best_omega_by_n_vs_cn.png")
    p.add_argument("--verbose", action="store_true")
    return p.parse_args()


def find_n_cn_dirs(base_dir: str, N: int, M: int) -> List[Tuple[int, float, str]]:
    ...
    for item in os.listdir(base_dir):
        ...
        for item2 in os.listdir(robots_dir):
            ...
            for item3 in os.listdir(env_dir):
                ...
                res.append((n_val, cn_val, cn_dir))
...
def build_heatmap(base_dir: str, N: int, M: int, sn: float, csv_name: str, verbose: bool=False):
    dirs = find_n_cn_dirs(base_dir, N, M)
    results: Dict[Tuple[int, float], Tuple[float, float]] = {}
    ns = set()
    cns = set()

    for n_val, cn, cn_dir in dirs:
        best_omega, best_perf = read_best_omega(cn_dir, sn, csv_name)
        if best_omega is None:
            continue
        results[(n_val, cn)] = (best_omega, best_perf)
        ns.add(n_val)
        cns.add(cn)

    if not results:
        raise RuntimeError("No results found. Check your parameters (N, M, sn) and directory layout.")
    ...
    for i, n_val in enumerate(ns_sorted):
        for j, cn in enumerate(cns_sorted):
            if (n_val, cn) in results:
                omega_mat[i, j] = results[(n_val, cn)][0]

    return ns_sorted, cns_sorted, omega_mat
...
def main():
    ...
    ns, cns, omega_mat = build_heatmap(
    ...
    df_out = save_csv(ns, cns, omega_mat, args.out_csv)
    plot_heatmap(df_out, args.out_png,
                 title=f"Best ω by (n, cn) | N={args.N}, M={args.M}, sn={args.sn}")

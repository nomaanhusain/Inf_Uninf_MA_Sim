...
class PlotGraph:
    def __init__(self, max_timestep, agent_count, majority_color_ratio, informed_ratio, personal_info_weight
                 , sensor_noise, communication_noise, color_change_timestep, neighbourhood_size, env_op):
        ...
        self.main_file_path = os.path.join(base_dir, "output_data", f"{agent_count}_robots_{self.neighbourhood_size}_neighbours", f"env_options_{env_op}",f"cn{self.communication_noise}_fixed",self.nested_folder)
    ...
    def execute_and_plot(self):
        ...
        # file.close()
        #TODO: Not 1000, should be dynamic based on swarm size
        merged_data = merged_data.iloc[self.color_change_timestep:] 
        # Plot averaged swarm_quality over timesteps
        plt.figure(figsize=(10, 5))
        plt.plot(merged_data.index.to_numpy(), merged_data['swarm_quality'].to_numpy(), label='Swarm Quality')
        plt.ylim(0.0, 1.0)
        plt.xlabel('Timestep')
        ...
        color_map = {'color_0': 'red', 'color_1': 'blue', 'color_2': 'green', 'color_3': 'purple', 'color_4': 'orange',
                     'color_5': 'aqua', 'color_6': 'teal', 'color_7': 'indigo', 'color_8': 'crimson', 'color_9': 'olive',
                     'color_10': 'rosybrown', 'color_11': 'lime', 'color_12': 'papayawhip', 'color_13': 'tan', 'color_14': 'thistle',
                     'color_15': 'saddlebrown', 'color_16': 'maroon', 'color_17': 'darkseagreen'}
        for color_col in merged_data.columns:
            if 'color' in color_col:
                plt.plot(merged_data.index.to_numpy(), merged_data[color_col].to_numpy(), label=color_col, color=color_map[color_col])
